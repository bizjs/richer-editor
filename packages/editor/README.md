...
通过 `features={{ outline: true }}` 启用外部大纲。大纲按文档顺序读取带稳定 ID 的标题，显示标题级别并跟随当前选区更新章节状态；点击项目可在可编辑或只读模式下定位并聚焦对应标题。大纲支持折叠、空文档提示和窄屏布局，标题编辑及受控文档替换会实时同步。大纲属于派生界面状态，不会创建 TOC 节点，也不会改变 `RicherDocument`。
